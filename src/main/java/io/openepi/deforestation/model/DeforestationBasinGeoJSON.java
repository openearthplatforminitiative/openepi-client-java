...
public class DeforestationBasinGeoJSON {
  ...
  public DeforestationBasinGeoJSON() {
  }

  public DeforestationBasinGeoJSON type(Object type) {
    this.type = type;
    return this;
  }
  ...
  public DeforestationBasinGeoJSON features(List<DeforestationBasinFeature> features) {
    this.features = features;
    return this;
  }

  public DeforestationBasinGeoJSON addFeaturesItem(DeforestationBasinFeature featuresItem) {
  ...
  public void setFeatures(List<DeforestationBasinFeature> features) {
    this.features = features;
  }
  ...
  public boolean equals(Object o) {
    ...
    DeforestationBasinGeoJSON deforestationBasinGeoJSON = (DeforestationBasinGeoJSON) o;
    return Objects.equals(this.type, deforestationBasinGeoJSON.type) &&
        Objects.equals(this.features, deforestationBasinGeoJSON.features);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to DeforestationBasinGeoJSON
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DeforestationBasinGeoJSON.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in DeforestationBasinGeoJSON is not found in the empty JSON string", DeforestationBasinGeoJSON.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DeforestationBasinGeoJSON.openapiFields.contains(entry.getKey())) {
      ...
      for (String requiredField : DeforestationBasinGeoJSON.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayfeatures.size(); i++) {
        DeforestationBasinFeature.validateJsonElement(jsonArrayfeatures.get(i));
      };
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DeforestationBasinGeoJSON.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DeforestationBasinGeoJSON' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<DeforestationBasinGeoJSON> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(DeforestationBasinGeoJSON.class));

       return (TypeAdapter<T>) new TypeAdapter<DeforestationBasinGeoJSON>() {
           @Override
           public void write(JsonWriter out, DeforestationBasinGeoJSON value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public DeforestationBasinGeoJSON read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of DeforestationBasinGeoJSON given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of DeforestationBasinGeoJSON
   * @throws IOException if the JSON string is invalid with respect to DeforestationBasinGeoJSON
   */
  public static DeforestationBasinGeoJSON fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, DeforestationBasinGeoJSON.class);
  }

  /**
   * Convert an instance of DeforestationBasinGeoJSON to an JSON string
   *
   * @return JSON string
   */
