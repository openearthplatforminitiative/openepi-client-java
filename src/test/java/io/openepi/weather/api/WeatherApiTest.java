...
public class WeatherApiTest {
    ...
    public void getLocationForecastTest() throws ApiException {
        ...
        when(api.getCompactForecast(lat, lon, null)).thenReturn(mockResponse);
        assertEquals(new BigDecimal("10"), api.getCompactForecast(lat, lon, null).getProperties().getTimeseries().get(0).getData().getInstant().getDetails().getAirTemperature());
    }
}
